...
def check_invalid_row(row: list) -> bool:
    """
        Функция ринимает на вход список row и проверяет, 
...
def get_no_invalid_data_index(data: list) -> list:
    """
        Функция принимает на вход список data, который представляет собой 
        список списков.
        Она проходит по каждой строке в списке data и вызывает функцию 
        check_invalid_row для проверки каждого элемента в строке. 
        Если хотя бы один элемент не соответствует своему шаблону, 
        индекс этой строки добавляется в список data_index. 
        В конце функция возвращает список индексов невалидных строк. 
        Таким образом, функция находит и записывает индексы невалидных 
        строк в исходном списке данных.
    """
    data_index = []
    index = 0
    for row in data:
