...
def read_file(filename: str) -> list:
    """
    Read file and check each string for valid regular expressions.
    filename - name of our variant
    return list_for_checksum - list of index strings where regular expression is not matched
    """
    first_string = True
    title = ""
    list_for_checksum = []
...
def check(string_dict: dict) -> bool:
    ...
    return True if value does not match the regular expression
    """
    for title, value in string_dict.items():
        if re.match(REGEX_PATTERNS[title], value) is None:
...
if __name__ == "__main__":
    ...
    hash_value = checksum.calculate_checksum(list_of_index_no_valid_strings)
    checksum.serialize_result(41, hash_value)
